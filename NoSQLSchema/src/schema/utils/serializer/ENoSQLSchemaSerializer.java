package schema.utils.serializer;
...
/**
 * Class used to serialize a NoSQLDataModel object in a human-readable way.
 * Implemented as a Singleton.
 * @author Alberto Hernández Chillón
 */
public class ENoSQLSchemaSerializer
{
	/**
	 * Singleton reference.
	 */
	private static ENoSQLSchemaSerializer instance;
	...
	/**
	 * Method used to get the ENoSQLSchemaSerializer instance.
	 * @return NoSQLSchemaSerializer instance.
	 */
	public static ENoSQLSchemaSerializer getInstance()
	{
		if (instance == null)
			instance = new ENoSQLSchemaSerializer();
	
		return instance;
	}

	/**
	 * Method used to stringify a NoSQLDataModel.
	 * @param theDataModel The NoSQLDataModel to be serialized.
	 * @return A String containing a serialized NoSQLDataModel.
	 */
	public String stringify(NoSQLDataModel theDataModel)
	{
		...
		result.append("NoSQLDataModel - name: " + theDataModel.getName() + ENDLINE);

		for (Entity entity : theDataModel.getEntities())
			result.append(stringify(entity, tabs));

		for (SchemaVersion schema : theDataModel.getSchemaVersions())
			result.append(stringify(schema, tabs));

		return result.toString();
	}

	/**
	 * Method used to stringify a SchemaVersion.
	 * @param schemaVersion The SchemaVersion to be serialized
	 * @param defTabs The number of tabs to apply to the serialization.
	 * @return A String containing a serialized schema.
	 */
	public String stringify(SchemaVersion schemaVersion, String defTabs)
	{
		...
		result.append(tabs + "SchemaVersion - root: " + "EntityVersion - versionId: " + schemaVersion.getRoot().getVersionId() + ENDLINE);

		if (!schemaVersion.getEntities().isEmpty())
		{
			result.append(tabs + TAB + "Entities: { ");

			for (Entity entity : schemaVersion.getEntities())
				result.append(entity.getName() + " ");

			result.append("}" + ENDLINE);
		}

		if (!schemaVersion.getEntityVersions().isEmpty())
		{
			result.append(tabs + TAB + "EntityVersions: { ");

			for (EntityVersion eVersion : schemaVersion.getEntityVersions())
				result.append(eVersion.getVersionId() + " ");

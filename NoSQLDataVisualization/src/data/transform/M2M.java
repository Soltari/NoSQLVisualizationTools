		...
		for (EntityVersion ev : baseEntity.getEntityversions())
		{
			...
			for (Property property : ev.getProperties())
			{
				...
				if (property instanceof Attribute)
				{
					...
					if (theType instanceof NoSQL_Schema.PrimitiveType)
					{
						if (name.equals("type") || name.equals("Type"))
						{
							type = Version_DiffFactory.eINSTANCE.createEntityType();
							((EntityType)type).setType(((NoSQL_Schema.PrimitiveType)theType).getName());
						}
						else
						{
							type = Version_DiffFactory.eINSTANCE.createPrimitiveType();
							((PrimitiveType)type).setType(((NoSQL_Schema.PrimitiveType)theType).getName());
						}
					}
					else if (theType instanceof Tuple)
					{
						...
						{
							if (tupleType instanceof NoSQL_Schema.PrimitiveType)
								((HeterogeneousTupleType)type).getType().add(((NoSQL_Schema.PrimitiveType)tupleType).getName());
